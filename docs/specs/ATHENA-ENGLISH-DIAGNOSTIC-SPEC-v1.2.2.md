# ATHENA ENGLISH DIAGNOSTIC SPEC v1.2.2

**Status:** Draft for final Codex implementation-gate re-review (corrective revision)
**Supersedes:** ATHENA ENGLISH DIAGNOSTIC SPEC v1.2.1 (surgical determinism patch only — not a redesign)
...
**Revision note:** v1.2.2 is a narrow corrective patch to v1.2.1, closing exactly the three normative defects raised by the final Codex implementation-gate re-review — SUM-002 (spelling classification is not deterministic), SUM-003 (copied-run contiguity is ambiguous), and MARK-002 (Unicode normalization form/version is unspecified). That gate's FAIL verdict was scoped entirely to these three findings; all seven v1.2→v1.2.1 findings (CONF-001, ROUTE-001, MARK-001, SUM-001, CLAIM-001, REC-001, SEC-001) remain resolved and are not reopened by this revision. This revision does not redesign the architecture, implement Day 1, or produce application code or migrations. Section 44 is the resolution matrix for this gate; Section 45 is the v1.2.1→v1.2.2 change log.
...
Precise, non-conflatable definitions. Implementers must not substitute one term for another. New/changed terms since v1.1 are marked **[v1.2]**; new/changed terms since v1.2 are marked **[v1.2.1]**; new/changed terms since v1.2.1 are marked **[v1.2.2]**.
...
| **`UNKNOWN_SPELLING_TOKEN`** **[v1.2.2]** | A normalized token that resolves to neither the canonical lexicon nor any frozen accepted list. Never auto-counted as an error; forces `PENDING_HUMAN_MARK` when it could affect the spelling threshold (Section 18). |
| **Copied run** **[v1.2.2, redefined]** | Four or more consecutive normalized response tokens that exactly match four or more consecutive normalized passage tokens at one contiguous passage-token span — contiguity is required in *both* sequences, not the response alone (Section 18). |
...
| Paraphrase **[v1.2.2]** | 2 | 2 marks when `copiedRunCount = 0`; 1 mark when `copiedRunCount = 1`; 0 marks when `copiedRunCount ≥ 2` — see below. |
...
| Spelling and punctuation **[v1.2.2]** | 1 | ≤2 confirmed `MISSPELLED` occurrences AND sentence boundaries are mechanically identifiable — see below. |
...
### Copied-run contiguity and counting [v1.2.2, SUM-003]

This resolves Codex's SUM-003 finding: v1.2.1's "four or more consecutive tokens appearing in the passage in the same order" was ambiguous between tokens consecutive in the response only (with the passage tokens preserved in order but separated by gaps) and tokens consecutive in *both* sequences — the two readings can produce different paraphrase marks for the same response. v1.2.2 requires contiguity in both sequences:

> A copied run exists only when four or more consecutive normalized response tokens exactly match four or more consecutive normalized passage tokens at one contiguous passage-token span, with no intervening passage tokens:
>
> ```text
> response[i : i+n] == passage[j : j+n], where n >= 4
> ```

A sequence that is consecutive in the response but reconstructed from non-contiguous passage tokens — an intervening passage word breaks the span — is **not** a copied run (Scenario Q). A sequence contiguous in both is a copied run (Scenario R).

Tokenization applies the shared `ATHENA-TEXT-NORMALIZATION-v1` base pipeline (NFKC, Unicode default case folding, whitespace collapse — Section 17), then, in this fixed order: punctuation removal, then word tokenization. An apostrophe within a contraction or possessive is retained as part of the token, not treated as removable punctuation; a hyphenated form tokenizes as one token, consistent with the word-count rule below. Function words are included; repeated common words alone do not form a run unless the full four-token sequence matches contiguously in both sequences. This algorithm is applied to the submitted response and the frozen passage.

**Overlapping and maximal spans.** Count maximal non-contained matching spans of length ≥4 only. A 7-token contiguous match is one copied run of length 7; its internal 4-, 5-, and 6-token subspans are not separately counted (Scenario R). Two candidate matching spans that overlap are merged into the maximal span; if they are instead separated by at least one non-matching token, they are counted as two distinct runs.

**Stored result (minimum):** `copiedRunCount`, `maxCopiedRunLength`, and `copiedRunSpans[]`, where each span records response token offsets, passage token offsets, and the matched token sequence (or its hash) — sufficient to audit the decision without re-running the algorithm.
...
### Spelling and punctuation mark [v1.2.2, SUM-002]

This resolves Codex's SUM-002 finding: v1.2.1 allowed spelling-error counting without defining how a token is determined to be misspelled, so the same token — a name, a contraction, an American variant, an unfamiliar-but-valid word — could be classified as an error by one implementation and as accepted by another, changing the mark. v1.2.2 introduces a versioned, deterministic spelling policy.
...
Day 1 uses British English (`en-GB`) as the canonical spelling baseline. This specification does not need to embed the lexicon itself, but the exact frozen asset and version an implementation loads must be fixed and recorded before the assessment leaves `HUMAN_APPROVED` and before Day 1 becomes `ACTIVE`; a spelling mark is never computed against an unversioned or "current" dictionary.

**Token classification order.** Tokens are classified in this fixed order; the first matching rule applies:

1. **Non-word tokens.** A punctuation-only token is ignored — it is not a word token and cannot be a spelling error.
2. **Proper nouns / names.** A token is exempt from spelling-error counting only if at least one of the following is true:
   * it appears, at the same normalized spelling, in the assessment passage;
   * it appears in a frozen assessment-specific `acceptedProperNouns` list;
   * it is explicitly approved in the frozen summary-marking asset.
   A marker must never invent a proper-noun exemption that does not satisfy one of these three deterministic conditions.
3. **Accepted contractions.** A contraction is accepted only if its normalized form is present in the frozen spelling lexicon or an assessment-specific accepted-form list — e.g. `don't`, `can't`, `it's`, `they're` — never on marker intuition about what "looks like" a valid contraction.
4. **British/American variants.** The Day 1 canonical standard is British English. Common standard American-English spelling variants contained in the frozen accepted-variant list — e.g. `color`/`colour`, `organize`/`organise`, `center`/`centre` — are accepted without penalty; a variant is a spelling error only if it is absent from that frozen list. Variant acceptance is a fixed rule for this six-week diagnostic, never a marker judgement call (Scenario P).
5. **Lexicon lookup.** After normalization:
   ```text
   token in canonical lexicon             → correct
   token in frozen accepted-variant list   → correct
   otherwise                               → UNKNOWN_SPELLING_TOKEN
   ```
   An unknown token is never automatically treated as an error.

**Unknown-token rule.** If any token affecting the ≤2-error threshold is `UNKNOWN_SPELLING_TOKEN` and is not resolved by a frozen assessment-specific accepted list, the spelling criterion is recorded as `PENDING_HUMAN_MARK` rather than being scored by model guess (Scenario O). This prevents arbitrary model judgement from silently deciding the mark.

**Error counting.** Only tokens deterministically classified as `MISSPELLED` (they fail every classification step above and are not `UNKNOWN_SPELLING_TOKEN`) count toward the ≤2-error threshold. Each occurrence counts as one spelling error — repeated occurrences of the same misspelled token are not collapsed. Three occurrences of one misspelled token produce `spellingErrors = 3`, unless the frozen lexicon/list resolves the token as valid.

**Mark rule:**

```text
≤ 2 confirmed MISSPELLED occurrences        → spelling mark eligible
> 2 confirmed MISSPELLED occurrences        → spelling mark not awarded
any unresolved UNKNOWN_SPELLING_TOKEN
  affecting the threshold                   → PENDING_HUMAN_MARK
```
...
Every Section E mark records `textNormalizationPolicyVersion`, `spellingPolicyVersion`, `lexiconVersion`, `assessmentAcceptedFormsVersion`, and `summaryMarkingPolicyVersion`. Historical marks retain the versions used at marking time; a later policy or version bump never silently reinterprets a historical mark (Governing Principle 13).
...
All three answers are `NO`. The patch is complete.

---

## Recommendation

**READY FOR FINAL CODEX v1.2.2 PASS/FAIL GATE**

All three findings (SUM-002, SUM-003, MARK-002) are closed under Section 44, with no row deferred. All seven prior v1.2→v1.2.1 findings remain resolved and are not reopened. Scenarios A–N remain valid under the patched rules; Scenarios O–S cover the three counterexamples directly. This is not authorization to implement Day 1. The sequence remains:
