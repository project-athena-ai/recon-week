# ATHENA ENGLISH DIAGNOSTIC SPEC v1.2.1

**Status:** Draft for final Codex implementation-gate re-review (corrective revision)
**Supersedes:** ATHENA ENGLISH DIAGNOSTIC SPEC v1.2 (surgical determinism patch only — not a redesign)
...
Precise, non-conflatable definitions. Implementers must not substitute one term for another. New/changed terms since v1.1 are marked **[v1.2]**; new/changed terms since v1.2 are marked **[v1.2.1]**.
...
| `summaryMarkingPolicyVersion` | Section E content-point model, paraphrase/copying rule, length handling, sentence-control checklist **[v1.2.1]** | Section 18 |
...
| Spelling and punctuation **[v1.2.1]** | 1 | ≤2 spelling errors AND sentence boundaries are mechanically identifiable — see below. |
...
### Spelling and punctuation mark [v1.2.1]

The 1 mark is awarded when the response has no more than two spelling errors **and** sentence boundaries are mechanically identifiable (condition 4 of the sentence-control checklist above). This mark and the sentence-control mark never double-count punctuation in contradictory ways: condition 4 is a single binary gate, evaluated once and read by both criteria; a punctuation deficiency that fails condition 4 affects both marks through that one shared evaluation, never through two independently-judged punctuation assessments. A spelling error is never itself recorded as a sentence-level grammar fault unless it independently causes one of the five enumerated fault categories (e.g. a spelling error that deletes a finite verb).
...
These are Day 1 prerequisites or later specifications. They do not permit a developer to substitute localStorage identity, existing Mission verification, arbitrary twin JSON, self-created eligibility, or model self-confidence.
...
## 42. Final Gate Finding Resolution Matrix
...
**READY FOR FINAL CODEX v1.2.1 PASS/FAIL GATE**

All seven findings (CONF-001, ROUTE-001, MARK-001, SUM-001, CLAIM-001, REC-001, SEC-001) are closed under Section 42, with no row deferred. Scenarios A–H remain valid under the patched rules; Scenarios I–N cover the seven counterexamples directly. This is not authorization to implement Day 1. The sequence remains:
